...
pd.set_option('future.no_silent_downcasting', True)   # 设置选项以启用未来版本的行为
...
def add_table(docx_obj, mb_obj, data_obj):
    ...
    data = {
        ...
        'cslx'  : data_obj['测试类型'],
        'hdz'   : data_obj['活动桩'],
        'cryj'  : '\n' + data_obj['插入语句'] if isinstance(data_obj['插入语句'], str) and '\n' in data_obj['插入语句'] else data_obj['插入语句'],
        'qjbl'  : '\n' + data_obj['创建全局变量'] if isinstance(data_obj['创建全局变量'], str) and '\n' in data_obj['创建全局变量'] else data_obj['创建全局变量'],
        'cssm'  : data_obj['测试说明'],
...
if __name__ == '__main__':
    ...
    excel_datas = pd.read_excel(excel_file, sheet_name=None, dtype=str)
    excel_datas = {sheet_name: fill_merged_cells(df) for sheet_name, df in excel_datas.items()} # 处理合并单元格
